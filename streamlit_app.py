...
st.markdown(f"""
    <div class="bmw-master-banner">
        <div>
            <div class="m-power-tag">M-POWER RED EDITION ($25 Fixed Risk Rule)</div>
...
# Always Visible Professional Sidebar Execution Panel with Corrected Lot Mapping
with st.sidebar:
    ...
    with st.form("trade_form"):
        ...
        # Stop Loss Selector with Marah's exact rules
        sl_options = ["25 pip", "30 pip", "40 pip", "50 pip"]
        selected_sl = st.selectbox("Stop Loss (SL)", sl_options)
        
        # Exact Lot Mapping requested by Marah: 25->0.1, 30->0.08, 40->0.06, 50->0.05
        lot_mapping = {
            "25 pip": 0.10,
            "30 pip": 0.08,
            "40 pip": 0.06,
        ...
        st.info(f"⚡ Auto Lot Size ($25 Risk): **{calculated_lot}**")
...
if submitted:
    ...
    save_trades()
    st.success("Trade recorded successfully with exact lot mapping!")
    st.rerun()
...
if st.session_state.trades:
    ...
    # Session Intelligence Breakdown Cards
    st.markdown("<br>", unsafe_allow_html=True)
    st.subheader("🌐 Session Intelligence (NYC vs London vs Asia)")
    
    sess_col1, sess_col2, sess_col3 = st.columns(3)
    sessions_list = ["NYC", "London", "Asia"]
    cols_list = [sess_col1, sess_col2, sess_col3]
    
    for i, sess_name in enumerate(sessions_list):
        sess_df = df[df['Session'] == sess_name]
        s_total = len(sess_df)
        s_wins = len(sess_df[sess_df['Result'] == 'Win'])
        s_losses = len(sess_df[sess_df['Result'] == 'Loss'])
        s_wr = (s_wins / s_total * 100) if s_total > 0 else 0
        
        with cols_list[i]:
            st.markdown(f"""
                <div style="background: linear-gradient(145deg, #09090c 0%, #160408 100%); border: 1px solid #3d0a14; border-top: 3px solid #ff1744; padding: 20px; border-radius: 14px; box-shadow: 0 8px 25px rgba(0,0,0,0.5);">
                    <h4 style="color: #ffffff; font-family: 'Orbitron', sans-serif; margin-top: 0; margin-bottom: 12px;">📍 {sess_name} Session</h4>
                    <p style="color: #fda4af; margin: 6px 0; font-size: 14px;">Total Trades: <b>{s_total}</b></p>
                    <p style="color: #34d399; margin: 6px 0; font-size: 14px;">Wins: <b>{s_wins}</b> | Losses: <b style="color: #f87171;">{s_losses}</b></p>
                    <p style="color: #f8fafc; margin: 6px 0; font-size: 14px;">Win Rate: <b>{s_wr:.1f}%</b></p>
                </div>
            """, unsafe_allow_html=True)
